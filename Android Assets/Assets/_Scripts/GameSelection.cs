...
public class GameSelection : MonoBehaviour
{
    ...
    // for scrollbars
    ScrollRect scrRect_Active;
    ScrollRect scrRect_History;

    // setting up references to objects/components
    // called form MainMenuButtons.FindNececaryGameObjects()
    public void set_SelectionObjectsReferences(GameObject passwordImg, InputField passwordInputFiled, 
        Text gamenameTag, Text notificationText, ScrollRect activeScrRect, ScrollRect historyScrRect)
    {
        ...
        scrRect_Active = activeScrRect;
        scrRect_History = historyScrRect;
    }
    ...
    public void Btn_LoadActiveGames(GameObject loadCircle)
    {
        ...
        reff.GetValueAsync().ContinueWith(task =>
        {
            ...
            else if (task.IsCompleted)
            {
                ...
                StartCoroutine(moveScrollBarToBottom(scrRect_Active));
    ...
    public void btn_load_historyGames(GameObject loadCircle)
    {
        ...
        reff.GetValueAsync().ContinueWith(task =>
        {
            ...
            else if (task.IsCompleted)
            {
                ...
                // sets scroll bar to bottom
                StartCoroutine(moveScrollBarToBottom(scrRect_History));
                // when done, disable loading image
                loadCircle.SetActive(false);
            }
        });
    ...
    // sets scroll bar to bottom - has to wait for the next frame to work
    IEnumerator moveScrollBarToBottom(ScrollRect scrollViewRect)
    {
